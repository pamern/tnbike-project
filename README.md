...
![Pipeline](reports/img/pipeline.png)
...
Note `processing_status` (cột trong `data/staging/staging_email_log.csv` và field `tnbike.email_log.processing_status`):

- `PROCESSING`: đang xử lý
- `SUCCESS`: xử lý thành công
- `NEEDS_REVIEW`: cần kiểm tra thủ công (dữ liệu/định dạng chưa đủ chắc chắn để auto)
- `FAILED`: lỗi xử lý (xem thêm `data/staging/staging_fail.csv` và `data/staging/staging_fail_summary.csv`)
...
- Adminer: `http://localhost:8080` (server: `postgres`, user/pass/db: `postgres/postgres`, database: `tnbike_db`)

Gỡ container (giữ data volume):

```powershell
docker compose down
```

Reset sạch DB (xoá cả volume `postgres_data`):
...
```powershell
docker cp .\backup\tnbike_db_backup.sql tnbike_postgres:/tmp/tnbike_db_backup.sql
docker exec -i tnbike_postgres psql -U postgres -d tnbike_db -f /tmp/tnbike_db_backup.sql
```
...
```powershell
docker cp .\sql\01_create_tables.sql tnbike_postgres:/tmp/01_create_tables.sql
docker cp .\sql\02_import_data.sql tnbike_postgres:/tmp/02_import_data.sql
docker cp .\sql\03_create_email_log.sql tnbike_postgres:/tmp/03_create_email_log.sql
docker cp .\sql\05_clean_province.sql tnbike_postgres:/tmp/05_clean_province.sql

docker exec -i tnbike_postgres psql -U postgres -d tnbike_db -f /tmp/01_create_tables.sql
docker exec -i tnbike_postgres psql -U postgres -d tnbike_db -f /tmp/02_import_data.sql
docker exec -i tnbike_postgres psql -U postgres -d tnbike_db -f /tmp/03_create_email_log.sql
docker exec -i tnbike_postgres psql -U postgres -d tnbike_db -f /tmp/05_clean_province.sql
```
...
python src/preprocessing/map_product_line.py
python src/preprocessing/map_customer_province.py
python src/preprocessing/update_customer_province_fact.py
```

Ghi chú: `map_product_line.py` sử dụng file mapping `data/processed/cleaned/map_product_line.csv` (các dòng `mapping_status=AUTO_MAPPED` sẽ được update vào DB).
